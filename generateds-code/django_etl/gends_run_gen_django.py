...
# if modified, also patch doc string above and in gends_generate_django.py.
DefaultSimpletype_array_size = 1000
...
def generate(options, schema_file_name):
    ...
    if not run_cmd(options, args):
        return
    args = [os.path.join('.', 'gends_generate_django.py'), '-f', ]
    if not options['class_suffixes']:
        args.append('--no-class-suffixes')
    if options['simpletype_array_size'] != DefaultSimpletype_array_size:
        args.append('--simpletype-array-size={}'.format(
            options['simpletype_array_size']))
    args.append(bindings_file_stem)
    if not run_cmd(options, args):
        return
...
def main():
    ...
    options['simpletype_array_size'] = DefaultSimpletype_array_size
    for opt, val in opts:
        if opt in ('-h', '--help'):
            usage()
        elif opt in ('-f', '--force'):
